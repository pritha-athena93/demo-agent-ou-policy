...
def build_round2_mandate(fields: dict, round1_comment_body: str, human_reply_raw: str) -> tuple:
    ...
    mandate = (
        ...
        "reason the workaround won't work.\n\n"
        "If the reply gives a specific, concrete value to grant (e.g. a named "
        "instance type, a named tag), you must call update_org_policy with it "
        "first -- do not skip straight to request_human_approval just because "
        "the change sounds significant. The registry/guardrail/allowlist "
        "checks are the ones deciding whether it's actually allowed, not you; "
        "calling update_org_policy and letting it come back blocked is the "
        "correct way to find that out, not a reason to avoid trying. Only "
        "call request_human_approval first, without trying update_org_policy, "
        "when the reply gives no concrete value to act on at all (e.g. still "
        "vague about which exact type/tag) or when update_org_policy's own "
        "result told you to escalate. Do not end this round without calling "
        "one of those two tools. State your final outcome once, in one or two "
        "sentences."
    )
    return mandate.strip(), was_flagged
